document.addEventListener("DOMContentLoaded", () => {
  ...
  // 14. Interactive Diagnostic Assessment Quiz
  const quizData = [
    {
      q: "Choose the correct preposition: 'She has been residing in Lucknow ________ 2018.'",
      options: ["for", "since", "from", "during"],
      ans: 1,
      explanation: "Use 'since' with an exact point in time (2018), and 'for' with a duration of time (e.g. 6 years)."
    },
    {
      q: "Select the word closest in meaning to 'LUCID':",
      options: ["Ambiguous", "Clear and easy to understand", "Overly complex", "Hesitant"],
      ans: 1,
      explanation: "'Lucid' means expressed clearly and easily understood—a hallmark of IELTS Band 7.5+ and executive communications."
    },
    {
      q: "Identify the grammatically correct sentence:",
      options: [
        "She suggested that we should took an early train.",
        "She suggested that we take an early train.",
        "She suggested us taking an early train.",
        "She suggested to take an early train by us."
      ],
      ans: 1,
      explanation: "The subjunctive mood following verbs of recommendation like 'suggest' uses the base verb form: 'suggested that we take'."
    },
    {
      q: "In professional corporate interviews, the STAR technique stands for:",
      options: [
        "Speaking, Tone, Accent, Response",
        "Situation, Task, Action, Result",
        "Story, Thesis, Argument, Rebuttal",
        "Skill, Timing, Attitude, Review"
      ],
      ans: 1,
      explanation: "STAR (Situation, Task, Action, Result) is the gold standard framework for structuring behavioral interview responses."
    },
    {
      q: "Which transition word best expresses an unexpected concession?",
      options: ["Furthermore", "Notwithstanding", "Consequently", "Namely"],
      ans: 1,
      explanation: "'Notwithstanding' functions formally like 'in spite of' or 'nevertheless', adding sophisticated nuance to your essays and discussions."
    }
  ];
  ...
  let recommendedCourseName = "IELTS";

  const quizQuestionTag = document.getElementById("quizQuestionTag");
  ...
  const quizRecommendationText = document.getElementById("quizRecommendationText");
  const quizEnrollBtn = document.getElementById("quizEnrollBtn");
  const quizRetakeBtn = document.getElementById("quizRetakeBtn");

  function renderQuizQuestion(index) {
    if (!quizQuestionBox || !quizOptions) return;
    const item = quizData[index];
    quizAnswered = false;

    if (quizQuestionTag) quizQuestionTag.textContent = `QUESTION ${index + 1} OF ${quizData.length}`;
    ...
    if (quizQuestionText) quizQuestionText.textContent = item.q;
    ...
    item.options.forEach((optText, optIdx) => {
      ...
      btn.innerHTML = `<span style="display:inline-flex; align-items:center; justify-content:center; width:22px; height:22px; border-radius:50%; background:var(--bg); border:1px solid var(--border); font-size:0.75rem; font-weight:700;">${String.fromCharCode(65 + optIdx)}</span> ${optText}`;
      
      btn.addEventListener("click", () => handleQuizOptionClick(optIdx, item));
      quizOptions.appendChild(btn);
    });
  }

  function handleQuizOptionClick(selectedIdx, item) {
    ...
    if (isCorrect) {
      ...
      if (quizExplanation) {
        quizExplanation.innerHTML = `<strong>✓ Correct!</strong> ${item.explanation}`;
        quizExplanation.classList.add("show");
      }
    } else {
      optButtons[selectedIdx].classList.add("wrong");
      optButtons[item.ans].classList.add("correct");
      if (quizExplanation) {
        quizExplanation.innerHTML = `<strong>✗ Not quite.</strong> ${item.explanation}`;
        quizExplanation.classList.add("show");
      }
    }
  ...
  function showQuizResults() {
    ...
    if (quizQuestionTag) quizQuestionTag.textContent = "ASSESSMENT COMPLETED";
    if (quizFinalScore) quizFinalScore.textContent = `${quizScore} / ${quizData.length}`;

    if (quizScore >= 4) {
      if (quizLevelTitle) quizLevelTitle.textContent = "Proficient / Band 7.5+ Ready!";
      if (quizRecommendationText) {
        quizRecommendationText.textContent = "Outstanding performance! You have a sophisticated grasp of grammar and academic vocabulary. We recommend our high-impact IELTS Mastery or Interview Preparation courses to unlock top international opportunities.";
      }
      recommendedCourseName = "IELTS";
    } else if (quizScore >= 3) {
      if (quizLevelTitle) quizLevelTitle.textContent = "Intermediate / Ready for Fluency Leap!";
      if (quizRecommendationText) {
        quizRecommendationText.textContent = "Good foundation! You communicate effectively but encounter occasional hiccups with nuanced grammar and sentence flow. Our Spoken English & Fluency or PTE Academic program will bridge this gap quickly.";
      }
      recommendedCourseName = "Spoken English";
    } else {
      if (quizLevelTitle) quizLevelTitle.textContent = "Foundational / Time to Build Confidence!";
      if (quizRecommendationText) {
        quizRecommendationText.textContent = "You are on the right track! Solidifying your core sentence structure, tense consistency, and prepositions will give you instant conversational confidence. We recommend starting with English Grammar Made Easy.";
      }
      recommendedCourseName = "English Grammar";
    }
  }

  quizEnrollBtn?.addEventListener("click", () => {
    selectEnquiryCourse(recommendedCourseName);
    openModal(enquireModal);
  });
  ...
  // 15. Daily Practice Hub Pronunciation Player
  const practiceAudioBtn = document.getElementById("practiceAudioBtn");
  const practiceAudioIcon = document.getElementById("practiceAudioIcon");

  practiceAudioBtn?.addEventListener("click", () => {
    if ("speechSynthesis" in window) {
      window.speechSynthesis.cancel();
      const textToSpeak = "Hit the nail on the head. Meaning: To describe exactly what is causing a situation or problem; to be one hundred percent accurate. Example: When Rahul explained why our presentation failed, he really hit the nail on the head.";
      ...
      if (practiceAudioIcon) practiceAudioIcon.setAttribute("data-lucide", "volume-x");
      refreshIcons();
      showToast("Playing authentic pronunciation...");

      utterance.onend = () => {
        if (practiceAudioIcon) practiceAudioIcon.setAttribute("data-lucide", "volume-2");
        refreshIcons();
      };
      utterance.onerror = () => {
        if (practiceAudioIcon) practiceAudioIcon.setAttribute("data-lucide", "volume-2");
